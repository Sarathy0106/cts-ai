...
class VectorStoreManager:
    ...
    def build_index(self, items):
        ...
        print("Priority Agent vector database built and saved.")
    ...
    for filename in os.listdir(dataset_dir):
        if filename.endswith(".csv"):
            ...
            try:
                ...
                print(f"[Priority RAG] Error loading {filename}: {e}")
    return items
    ...
    for filename in os.listdir(doc_dir):
        if filename.endswith(".pdf"):
            ...
            try:
                ...
                print(f"[Priority RAG] Error loading PDF {filename}: {e}")
    return documents


def retrieve_evidence(query, top_k_docs=3, top_k_data=5):
    store = VectorStoreManager()
    if store.index is None and not store.load_index():
        print("Priority Agent: Vector database not found. Building now...")
